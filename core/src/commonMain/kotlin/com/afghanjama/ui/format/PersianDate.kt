...
object PersianDate {
    ...
    fun startOfJalaliDay(jy: Int, jm: Int, jd: Int): Long {
        ...
        return LocalDateTime(g[0], g[1], g[2], 0, 0, 0)
    ...
    fun todayJalali(millis: Long = nowMillis()): IntArray =
    ...
    private fun jalaliOf(millis: Long): Triple<IntArray, Int, Int> {
        ...
        val j = gregorianToJalali(dt.year, dt.monthNumber, dt.dayOfMonth)
        return Triple(j, dt.hour, dt.minute)
    }

    /** «۱۴۰۴/۰۴/۱۵» — عددی و فشرده برای لیست‌ها. */
    fun short(millis: Long): String {
        val (j, _, _) = jalaliOf(millis)
        return "${j[0]}/${j[1].pad2()}/${j[2].pad2()}".toPersianDigits()
    }

    /** «۱۴۰۴/۰۴/۱۵ ۱۰:۳۰» — با ساعت. */
    fun shortWithTime(millis: Long): String {
        val (j, h, m) = jalaliOf(millis)
        return "${j[0]}/${j[1].pad2()}/${j[2].pad2()} ${h.pad2()}:${m.pad2()}".toPersianDigits()
    }
     ...
     */
    fun recentMonths(count: Int, now: Long = nowMillis()): List<Pair<String, String>> {
    ...
    fun csv(millis: Long): String {
        val (j, h, m) = jalaliOf(millis)
        return "${j[0]}/${j[1].pad2()}/${j[2].pad2()} ${h.pad2()}:${m.pad2()}"
    }
}
