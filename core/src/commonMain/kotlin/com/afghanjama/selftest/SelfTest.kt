...
import com.afghanjama.util.toFixed
...
fun checkBreakSchedule(delayUntilNext: (Int, Int, Long) -> Long): List<CheckResult> {
    ...
    val now = nowMillis() / minute * minute + 42_000L
...
fun checkPaperGeometry(papers: List<PaperSpec>): List<CheckResult> {
    ...
    papers.forEach { p ->
        ...
        s.isTrue(
            ...
            "جمعِ ستون‌ها ${sum.toFixed(1)} شد ولی عرضِ محتوا ${p.contentW} است",
        ...
        s.isTrue(
            ...
            tightest >= 0f,
            "«$tightestName» ${(-tightest).toFixed(1)} نقطه جا کم دارد",
            "تنگ‌ترین ستون «$tightestName» با ${tightest.toFixed(1)} نقطه فضای اضافه"
        )
    }
    return s.results
}
...
fun checkBackupArchive(
    ...
    val sqliteHead = "SQLite format 3 ".encodeToByteArray()
    s.eq("فایلِ zip شناخته می‌شود", "ZIP", detect(zipHead))
    s.eq("پشتیبانِ قدیمیِ SQLite شناخته می‌شود", "RAW_DB", detect(sqliteHead))
    s.eq("فایلِ آشغال ناشناس می‌ماند", "UNKNOWN", detect("hello world!!!!!".encodeToByteArray()))
...
fun checkOrderCycle(): List<CheckResult> {
    ...
    run {
        ...
        s.eq("هیچ سندِ ناترازی در حذفِ سفارشِ خرج‌کاردار نبود", null, r["__UNBALANCED__"])
    }
    ...
    run {
        ...
        s.eq("هیچ سندِ ناترازی در حذفِ سفارشِ دوخته‌شده نبود", null, r["__UNBALANCED__"])
    }
