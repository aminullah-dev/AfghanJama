...
class Repo(private val db: Db) {
    ...
    suspend fun cancelAssignment(assignmentId: Long) {
        ...
        val order = db.orderDao().getById(uuidOf(a.orderId)) ?: return
    ...
    private suspend fun completeAssignmentTx(
        ...
        val done = a.copy(qty = delivered, status = "DONE", doneAt = nowMillis(), quality = quality.trim())
        ...
        val orderFetched = db.orderDao().getById(uuidOf(a.orderId)) ?: return
    ...
    private suspend fun settleTailorWagesTx(
        ...
        db.tailorWageDao().settleForTailor(tailorLabel, nowMillis())
    ...
    suspend fun addCustomerPayment(payment: CustomerPayment) {
        ...
        val name = payment.customerName.ifBlank {
            runCatching {
                db.orderDao().getById(uuidOf(payment.orderId))?.customerName
            }.getOrNull().orEmpty()
        }
     ...
     *
     * **ولی آن قاعده یک پیش‌فرض دارد که امروز درست نیست.**
     *
     * صندوقِ خروجی نوشته می‌شود (۹ جا در همین کلاس) و هیچ‌کس آن را
     * نمی‌خوانَد: نه `markProcessed` صدا زده می‌شود، نه `pending`.
     * پس `processedAt` هرگز پر نمی‌شود، و شرطِ «فقط پردازش‌شده‌ها» یعنی
     * **هیچ سطری هرگز پاک نمی‌شود**. جدول بی‌مرز رشد می‌کرد و هر
     * پشتیبان‌گیری هم آن را با خود می‌بُرد — روی گوشیِ ارزانِ کارگاه،
     * سال به سال.
     *
     * حالا خودش را با واقعیت تطبیق می‌دهد: اگر این صندوق **تا به حال**
     * مصرف‌کننده‌ای داشته، همان قاعدهٔ محافظه‌کارانه سرِ جایش است؛ اگر
     * هرگز نداشته، مصرف‌کننده‌ای هم نیست که محافظت بخواهد و سن تنها
     * ملاک است.
     *
     * یعنی روزی که Dispatcher ساخته شود، این تابع بی هیچ دست‌کاری به
     * رفتارِ امنِ قبلی برمی‌گردد.
     */
    suspend fun pruneEvents(now: Long = nowMillis()): Int {
        val cutoff = now - RETENTION_MONTHS * 30L * 24 * 60 * 60 * 1000
        val dao = db.domainEventDao()
        return if (dao.processedCount() > 0) dao.prune(cutoff) else dao.pruneByAge(cutoff)
    }
    ...
    private suspend fun renameMaterialTx(id: Long, newName: String, newUnit: String): Boolean {
        ...
        db.materialStockDao().rename(id, nm, un, nowMillis())
    ...
    suspend fun outstandingAdvance(type: String, name: String): Long =
        partyBalanceUpTo(type, name, nowMillis()).coerceAtLeast(0L)
    ...
    suspend fun checkIn(employee: String) {
        ...
        db.attendanceDao().insert(AttendanceRecord(employee = emp, checkIn = nowMillis()))
    }
