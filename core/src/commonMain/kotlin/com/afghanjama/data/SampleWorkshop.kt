...
object SampleWorkshop {
    ...
    /**
     * دکمهٔ «ساختنِ کارگاهِ نمونه» در چه حالی است.
     *
     * **شروعِ استاندارد.** هر کارگاهِ تازه اول همین نمونه را می‌سازد و بعد
     * نام‌ها، قیمت‌ها، آدم‌ها و مشتری‌ها را از آنِ خودش می‌کند. پس کارگاهِ
     * کاملاً خالی ([FIRST_RUN]) پیش از هر کاری به این صفحه فرستاده می‌شود —
     * فعلاً روی اندروید، تنها جایی که «پاک‌کردن کارها و حساب‌ها» دارد و
     * سفارش‌ها و پولِ نمونه را می‌شود بعد از شخصی‌سازی بیرون برد.
     *
     * **قفل.** از لحظه‌ای که کارگاه چیزی دارد — همین نمونه، یا دادهٔ
     * خودش — ساختنِ دوباره رمزِ ورود می‌خواهد. دوباره ساختن چیزی را به
     * نمونه برنمی‌گرداند؛ یک نسخهٔ دیگر از سفارش‌ها و پولِ ساختگی را روی
     * کارِ واقعی می‌ریزد. یک لمسِ اتفاقی نباید این کار را بکند.
     */
    enum class Gate {
        /** هنوز معلوم نیست کارگاه چیزی دارد یا نه. */
        CHECKING,

        /** کارگاه خالی است: اول باید نمونه ساخته شود. رمز لازم نیست. */
        FIRST_RUN,

        /** کارگاه داده دارد و رمز زده نشده. */
        LOCKED,

        /** کارگاه داده دارد و رمز همین حالا درست زده شد. */
        OPEN
    }

    /** [hasData] `null` یعنی هنوز پرسیده نشده. */
    fun gate(hasData: Boolean?, unlocked: Boolean): Gate = when {
        hasData == null -> Gate.CHECKING
        !hasData -> Gate.FIRST_RUN
        unlocked -> Gate.OPEN
        else -> Gate.LOCKED
    }
