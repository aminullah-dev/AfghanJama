...
class IosNavStack(start: String) {
    ...
    /**
     * جایگزین کردنِ صفحهٔ رویِ پشته با [route].
     *
     * **برای چیپ‌های مرحله (برش / دوخت / نظارت).** آن سه هم‌سطح‌اند،
     * نه تودرتو: رفتن از دوخت به نظارت نباید پشته را بلندتر کند، وگرنه
     * دکمهٔ برگشت کاربر را در زنجیره‌ای از مرحله‌ها عقب می‌بَرد.
     *
     * روی اندروید همین کار با `popUpTo(CUTTING) { inclusive = true }`
     * انجام می‌شود. تفاوتِ ریزش عمدی است: آنجا اگر «برش» در پشته نباشد
     * `popUpTo` هیچ نمی‌کند و پشته بلند می‌شود؛ اینجا همیشه جایگزین
     * می‌شود، که همان چیزی است که آن بند می‌خواست.
     */
    fun replace(route: String) {
        if (stack.last() != route) stack[stack.size - 1] = route
    }
