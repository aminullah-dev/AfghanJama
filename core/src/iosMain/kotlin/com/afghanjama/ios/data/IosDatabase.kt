...
/*
 * دفترِ کارگاه روی iOS.
 *
 * **چرا `@Database`ِ سوم و نه همان دوتای موجود.** همان دلیلی که
 * `DesktopDatabase` را از `AppDatabase` جدا کرد: مهاجرت‌های اندروید روی
 * `SupportSQLiteDatabase` نوشته شده‌اند — API فقط-اندرویدیِ Room — و
 * بردنشان به کدِ مشترک یعنی بازنویسیِ کدی که دادهٔ واقعیِ کارگاه را ارتقا
 * می‌دهد.
 *
 * **و آنچه مشترک است، همه چیزِ مهم است:** هر ۴۲ موجودیت، تبدیل‌گرها و
 * عددِ نسخه از `:core` می‌آیند و این فایل فقط اعلانشان است. یعنی اسکیما
 * یک جا توصیف می‌شود و فایلِ دفتر بینِ گوشی و پی‌سی و آیفون جابه‌جا
 * می‌شود.
 *
 * **مهاجرت روی iOS هنوز نیست** — مثلِ ویندوز در روزِ اولش. این دیتابیس
 * تازه ساخته می‌شود و مستقیم روی [DB_VERSION] می‌نشیند. اولین باری که
 * اسکیما عوض شود، اینجا هم باید مهاجرت اضافه شود.
 */

/**
 * سازندهٔ دیتابیس — **روی Kotlin/Native اجباری است، نه اختیاری.**
 *
 * روی JVM و اندروید، Room کلاسِ `_Impl`ِ تولیدشده را سرِ اجرا با بازتاب
 * پیدا می‌کند. Kotlin/Native بازتاب ندارد، پس پیوند باید سرِ کامپایل
 * بسته شود: این `expect object` را KSP برای هر هدف با یک `actual`
 * پُر می‌کند.
 *
 * بدنه‌اش عمداً خالی است؛ نوشتنِ چیزی در آن هم بی‌فایده است چون
 * جایگزین می‌شود.
 */
...
abstract class IosDatabase : RoomDatabase(), Db {
    /*
