...
compose.desktop {
    application {
        ...
        nativeDistributions {
            ...
            /*
             * همان نسخه‌ای که اپِ اندروید دارد — از `gradle.properties`.
             *
             * تا دیروز اینجا `"1.0.0"` ثابت بود و اندروید `"1.0"`. دو
             * عدد برای یک نسخه، و هیچ‌کدام از دیگری خبر نداشت.
             *
             * **و مهم‌تر:** این عدد هرگز بالا نمی‌رفت. ویندوز MSIِ تازه
             * را وقتی جایگزینِ نصب‌شده می‌کند که `ProductVersion` بیشتر
             * باشد؛ با عددِ یکسان، نصب رد می‌شود — روی پی‌سی‌ای که دفترِ
             * حسابِ کارگاه رویش است.
             */
