...
# ── ۵. فایلِ گزارشِ خرابی ────────────────────────────────────────
# مرحلهٔ آخرِ آزمون یک خرابیِ ساختگی می‌کارد و می‌سنجد که روی صفحه بیاید.
# اگر نامِ فایل از `CrashLog` جدا بیفتد، اپ فایل را نمی‌بیند، صفحهٔ گزارش
# نمی‌آید و آزمون قرمز می‌شود — که خوب است — ولی علتش گنگ می‌ماند. اینجا
# صریح گفته می‌شود.
#
# مثلِ بندِ نامِ دیتابیس، «آیا این نام جایی هست؟» کافی نیست: نامِ فایل در
# مسیرهای موقت (`/tmp`, `/data/local/tmp`) هم می‌آید و آن‌ها اهمیتی
# ندارند. فقط **مقصد داخلِ اپ** سنجیده می‌شود. این را با خراب کردنِ
# `CRASHFILE` امتحان کردم و نسخهٔ اولِ این بند سبز ماند.
m = re.search(r'const val FILE\s*=\s*"([^"]+)"', _src.read("util/CrashLog.kt"))
m_dest = re.search(r'CRASHFILE="[^"]*/([^"/]+)"', smoke_code)
if m and not m_dest:
    fail.append("اسکریپت مسیرِ فایلِ خرابی (CRASHFILE) را ندارد.")
elif m and m_dest and m_dest.group(1) != m.group(1):
    fail.append(
        f"اسکریپت فایلِ خرابی را «{m_dest.group(1)}» می‌نویسد ولی CrashLog "
        f"«{m.group(1)}» را می‌خواند — پس صفحهٔ گزارش هرگز نمی‌آید."
    )
